...
This example demonstrates chained HTTPS calls between simple Flask apps that
manually forward `X-ProxyLens-*` headers on their downstream requests.
...
All three addon instances run with `max_concurrent_requests_per_host=1`.

## What This Shows

Because the applications propagate ProxyLens headers themselves, the full
`browser -> app1 -> app2 -> app3` chain should appear as one connected trace
even without OpenTelemetry propagation.

- simple Flask applications
- outbound downstream calls made with `requests`
- explicit manual forwarding of `X-ProxyLens-HopChain` and `X-ProxyLens-RequestId`
- one connected ProxyLens trace across the full chain

## Start

From `examples/chained-apps-with-manual-propagation/`:
